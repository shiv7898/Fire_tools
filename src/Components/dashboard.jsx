...
export default function Dashboard({ userData }) {
  ...
  console.log("Data Response in Dashboard....", dataResponse);
  ...
  useEffect(() => {
    function handleClickOutside(e) {
      const clickedInSidebar = sidebarRef.current && sidebarRef.current.contains(e.target);
      const clickedInLocationSidebar = locationSidebarRef.current && locationSidebarRef.current.contains(e.target);
      const clickedInPopup = popupWrapperRef.current && popupWrapperRef.current.contains(e.target);

      // Only close sidebars when click is outside sidebar, location sidebar and popup
      if (!clickedInSidebar && !clickedInLocationSidebar && !clickedInPopup) {
        setIsOpen(false);
        setLocationSidebarOpen(false);
      }
    }
  ...
  const userDataForPopup = userData;

  /* 🔹 Calculate subscription info dynamically */
  const calculateSubscription = (generatedAt, totalDays) => {
    console.log("Calculating subscription from:", generatedAt, totalDays);
    ...
    console.log("Days left calculation:", daysLeft);
  ...
  useEffect(() => {
    if (!userDataForPopup) return;



    if (userDataForPopup?.subscription?.generated_at) {
      const result = calculateSubscription(
        userDataForPopup.subscription.generated_at, userDataForPopup.subscription.time_period
      );
  ...
  return (
    <div className="main-container" onClick={()=>setMobileMenuOpen(false)}>
      ...
      <div className="mobile-topbar">
        ...
        {mobileMenuOpen && (
          ...
          >
            {sidebarMenuItems.slice(0).map((item) => {
              const content = (
                <>
                  {item.icon}
      ...
      <div className="outer-toggle-container">
        ...
        <div className={`sub-maincontainer1 ${isOpen ? "shrink" : ""}`}>
          <div className="tower-container" style={{ position: "relative" }}>
            <div
              className={`location-sidebar${locationSidebarOpen ? " open" : ""
                }`}
            ...
            <div className="tower-heading" ref={popupWrapperRef}>
          ...
          {inactivePanel === true && (
            <div className="event-container1">
              <InactivePanel ActiveInactive={panelUseRef.current} />
            </div>
          )}
